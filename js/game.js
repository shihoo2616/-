...
${[["dashboard","대시보드"],["career","내 선수"],["match","경기"],["training","훈련"],["club","구단"],["transfer","이적시장"],["national","국가대표"],["records","기록실"],["hall","명예의 전당"]].map(([v,t])=>`<button class="${view===v?"active":""}" data-view="${v}">${t}</button>`).join("")}</div><div class="sidebar-footer">로컬 저장 방식<br>GitHub Pages에서 서버 없이 플레이 가능</div></aside>`}
...
function match(){const c=CLUBS.find(x=>x.name===S.club);return `<div class="card"><h3>${S.season}시즌 · ${S.week}주차</h3>${matchCard(c)}<hr><div class="grid g3"><div><span class="muted small">선발 경쟁</span><div class="metric">${Math.min(99,Math.round(S.trust*.7+S.form*.3))}%</div></div><div><span class="muted small">출전 예상</span><div class="metric">${S.energy>35?"선발":"벤치"}</div></div><div><span class="muted small">상대 전력</span><div class="metric">${CLUBS[(CLUBS.findIndex(x=>x.name===S.club)+S.week)%CLUBS.length].strength}</div></div></div></div>`}
...
function render(){if(!S.started&&view!=="new")view="new";let body=view==="new"?newView():({dashboard:()=>dashboard(),career:()=>career(),match:()=>match(),training:()=>training(),club:()=>club(),transfer:()=>transfer(),national:()=>national(),records:()=>records(),hall:()=>hall()}[view]||dashboard)();shell(body,view==="new"?"NEW FOOTBALL LIFE":({dashboard:"커리어 대시보드",career:"내 선수",match:"MATCH CENTER",training:"TRAINING",club:"MY CLUB",transfer:"TRANSFER MARKET",national:"NATIONAL TEAM",records:"CAREER RECORDS",hall:"HALL OF FAME"}[view]));app.insertAdjacentHTML("beforeend",eventModal());const st=document.querySelector("#start");if(st)st.onclick=start}
